...
that precede implementation are tagged `(DESIGN)`; entries recording the
empirical outcome of a training run (no source change) are tagged `(FINDING)`.

---

## 2026-08-11 CDT — v5 lineage consolidated into one series; run tracking gains a compute axis (pending commit)

The v5 training record was split across two systems that shared no step axis: `data/v5.csv` held only the first three segments (original M5 machine, cum 1,000–99,901), while five continuation segments existed solely as probe JSONL in `~/Downloads/v5-continue-bundle/monitor/`. Both are now **one 856-row series** spanning **8 segments / 2 machines, cum step 1,000 → 859,769, 110.95M corpus games (≈5.30 epochs)**. No training was started or stopped; nothing was deleted.

- **Three x-axes, kept distinct** (architectural decision, recorded in CLAUDE.md): **step** (`cum_step`), **time** (`elapsed_train_sec`, plus a new raw `wall_sec`), and **compute** (new `games_fed`, cumulative corpus games measured from the runner's `games=`). Time is **never normalized across devices** — v5's by-time curve genuinely breaks 2.59× at cum 100,320 (M5 1.327 s/step → M4 Pro ~3.43 s/step) and that break is hardware, left visible. With batch 4096 / replayRatio 0.48 constant on both machines, a "normalized seconds" axis would reduce to `steps × reference_ms`, adding nothing while destroying the record of real cost.
- **`_schema.py`** — `FIELDS` gains `wall_sec` + `games_fed` (appended, so existing CSVs keep parsing).
- **`replay.py`** — new `LogIndex` parses each session log **once** into step-keyed metrics/games/timestamps, replacing a per-lookup full-file rescan (importing 761 probes against 6.4 GB of logs would have re-read ~1 TB; the whole import now runs in ~12 s). `_clamped_timeline` additionally returns the **uncapped** walk so `wall_sec − elapsed_train_sec` exposes exactly what the sleep clamp discarded — measured at 0.00 h on four of five segments and **20.54 h (5.7%)** on run 2's 15-day span. New `import-probes` subcommand folds external probe JSONL into a run's CSV, joining log metrics at the same step; it **refuses any probe whose `modelID` ≠ its segment's registry `model_id`** (a monitor that trusted filenames once re-probed nine month-old files and published a fabricated curve). New `_games_at` carries the same staleness guard as `_metrics_at`, and the `[REPLAY] done:` summary is indexed so each segment's final row gets its exact games total rather than a reading up to 50 steps stale.
- **`registry.json`** — `runs.v5` extended from 3 to 8 segments, each with `device`, `model_id`, `games_base`, and pinned `elapsed_base_sec` / `wall_base_sec`. Seg 3's base is **100,320** (`60,901 + 39,419`, the entry checkpoint's own `training_step`), not v5.csv's last row; seg 7's is **857,769** because run 5 resumed from run 4 at step 46,000, not run 4's end.
- **`master.py`** — third axis wired through: "by compute" toggle, `Mgames` table column, tooltip formatting. Verified the other 28 runs' step/time/metric series are byte-identical afterward, and v5's original 95 rows unchanged as a prefix.
- **`ckpt_inventory.py`** (new) — identifies checkpoints by safetensors `__metadata__` rather than filename, because segment-local step numbering means names repeat and get overwritten (**four distinct files have been named `v5-cont-replay-step1000.safetensors`**). Manifest of all **650** surviving checkpoints (616 distinct by content) with SHA-256.
- **`documentation/v5-lineage.md`** (new) — the consolidated record: segment chain with verified modelID links, every offset derivation, axis semantics, records (**pElo 1770.46 @ cum 638,826**, NLL 1.8445 @ cum 636,826 — both **+100,320** from the retired monitor's figures), the checkpoint inventory, and run 4's shard-permission truncation with why run 5 forked at step 46,000.
- **Corrects `archive/MANIFEST.md` §3**, which claimed `preserved-best/` held the only surviving weights: run 1's 268 enumerated checkpoints survive complete in `run1/`, and 231 of run 2's at the bundle root.
- **Boundary fix in `SegTime`** — `seg_for()` returns the last segment whose base ≤ cum, which mis-resolves any mark sitting exactly on a boundary: every segment's final step *equals* the next segment's `cumstep_base`, and seg 7's base additionally falls **inside** seg 6's span (run 5 forked from run 4 at step 46,000). All four boundary rows were taking `wallclock_iso` from the *following* run's log — off by 21 s, 46 s, 8 h 07 m and 9 h 56 m. `elapsed_and_clock` / `wall_at` now accept an explicit `segment`, which `import_probes` and `recompute_elapsed` pass; `seg_for` documents the hazard. (`elapsed`/`wall` were unaffected only because the pins are defined to coincide at exactly those points.) Verified by re-importing all five segments: exactly 4 fields changed, all `wallclock_iso`, and cum 857,769 now reads `2026-08-04T12:22:19` — matching that checkpoint's own header timestamp.
- **Guards against destructive re-runs** — `migrate v5` rebuilt the CSV from the markdown doc and would have **deleted all 761 imported rows**, and separately blanked the 80 measured `elapsed_train_sec` values for segments 0–2 (whose logs live on the other machine, so SegTime resolves them to ""). It now rebuilds only doc-sourced segments, carries later segments through untouched, and never replaces a real reading with a blank. `recompute` would likewise have moved `elapsed_train_sec` while leaving `wall_sec` stale, making their difference a meaningless "clamp measurement"; both now move together. Both verified in a `DCM_DASH_ROOT` sandbox.
- **Corpus manifests restored** — `master.py` derives the `epochs` column and the corpus summary table from `~/Library/Application Support/DrewsChessMachine/Corpora/<id>/corpus.json`, which this Mac never had (it only ever read the bundle's shards via `--replay-corpus`, which registers nothing). Regenerating the dashboard here therefore blanked `epochs` for all 14 corpus-backed runs and emptied the corpus table — silently, since `_load_corpora` skips a corpus whose directory is gone. The three manifests were copied across; every run's `epochs` is restored byte-identical (qeu8 9.6804, qeu8e 10.3591, mini2b 1.5765), and v5 now reads **5.291 epochs** for the full lineage — independently corroborated by the new compute axis at 110,949,479/20,935,171 = **5.300**. Worth noting the fragility: run config is versioned in `registry.json` but corpus config is per-machine, so the same repo renders differently on different machines with no warning.
...
New preset `v5_5block_7x7_lnout` = the `v4_5block_7x7` recipe (authored by *mutating* it, so it tracks future v4 changes) + LayerNorm on every block; adds 2·C params/block (γ/β): 8,445,748 → **8,447,028**. `architectureVersionLabel` returns 5 when any group carries an output norm (post-dates the pre/post v3/v4 split, so it wins). `weightTensorPlan` appends `res_ln.weight`/`res_ln.bias` LAST in the block, mirroring the builder's append order (the index-aligned save/load contract); `groupSummary` gains `, out:layer_norm` only when present, so v3/v4 summaries — and their golden-string tests — stay byte-identical. Build-New-Model gets an output-norm picker + an `ArchitectureDiagramView` line. New tests pin the plan/param count, tensor order, version label, and a GPU build+forward. Empirically the v5 run cleared the 5–9k inference-collapse zone every v4 variant died in and reached new probe-pElo highs (≥1415).
...
`ProbeModelCLI` now emits **`policy_logit_abs_max`** (mean over the battery of the per-position max |raw policy logit|, matching the trainer's `pLogitAbsMax` diagnostic) and **`policy_logit_abs_max_peak`** (worst single position) — a probe-side read on logit blow-up. `SessionController.runBatch` returns the per-position array (`logitAbsMaxPerPos`); error/empty paths return `[]` and the emitter slices defensively.

Files: `App/NewModelCLI.swift` (new), `App/DrewsChessMachineApp.swift`, `App/ProbeModelCLI.swift`, `App/SessionController+TacticalProbe.swift`.

## 2026-06-23 CDT — Charts: sampled-batch overlays + single-flight analyses (`52104f5`, `68f66d7`)

Two trainer-side series now overlay the self-play tiles (`52104f5`): the position-weighted **mean game length** and **realized draw rate** of the minibatches the trainer samples from the replay buffer, drawn in blue over the existing orange self-play series so "what self-play produces" reads against "what the trainer consumes." The realized batch composition is read off `ReplayBuffer.lastSamplingResult()` in `trainStep` phase 1 (valid every step — the uniform fast path tallies it too), threaded through `TrainStepTiming` into two new 512-step rolling windows in `TrainingLiveStatsBox` (NaN-guarded for the random-data sweep path), and carried through `TrainingChartSample`/`TrainingBucket` to two new tiles (`GameLengthChart`, `DrawRateChart`, replacing the prior single-series `MiniLineChart`s). Also on the `[STATS]` line as `sampBatch=(len=.. D=..)`. The two series are deliberately not level-comparable — self-play is per-game over a 60 s window, the batch series per-position over a 512-step window, so the batch lines sit higher and diverge further when the draw-cap / length-tilt sampling constraints are active, which is the signal.

The Debug-menu **analyses are now single-flight** (`68f66d7`): `beginAnalysis`/`endAnalysis` on `SessionController` gate on an observable `runningAnalysisLabel`, so a second press while one is running gets a non-blocking "already running" alert and bails, while the first pushes an "Analyzing <label>…" status row. Each entry takes the guard after its precondition and clears it from the detached task's `defer`, so it ends on every exit path. (`dd879b4` was incidental build-counter/pbxproj normalization — no functional change.)

## 2026-06-20 CDT — Self-play game corpus: recording, offline replay, PGN import (`f0c0012`, `0ad27b3`, `c4243b5`, `461c95c`, `049e94f`, `5a93c54`)

The corpus record/replay feature designed in ROADMAP.md ships across six commits (logic-reviewed under a no-build constraint at commit time). An architecture-independent **game corpus** stores move lists, not encoded tensors (~2 bytes/ply vs ~7.7 KB/pos for encoded basic30 — 80M plies/h is ~160 MB/h, not ~600 GB/h), so the same corpus trains any network shape and external PGN imports later.

- **Format + store (`f0c0012`).** `GameRecord` = start position + 2-byte packed moves + W/D/L outcome (+ optional termination reason); the `PackedMove` codec uses an explicit-switch promotion map so the on-disk format can't drift if `PieceType` is reordered. Shard files: fixed 256-byte front header, CRC-32-framed records, a 64-byte SHA-256 trailer appended at seal (pure-append; seal facts live in the trailer, not backfilled). Per-record CRC for open-log recovery, whole-shard SHA for sealed integrity. `GameCorpus` is a standalone `Corpora/` store (never embedded in a session folder) with one provenance-only `corpus.json` (append-only `sources[]`), soft-byte-target shard rotation cut at whole-game boundaries, and crash recovery on open.
- **Recording (`0ad27b3`).** Gated by a new default-off `recordSelfPlayGames` `@TrainingParameter` (registry 59→60), `CorpusRecorder` tees every kept (post-draw-filter) self-play game off `BatchedSelfPlayDriver.handleGameEnds` to the single-writer corpus via a private serial queue — best-effort, never blocking the hot path or propagating an append failure into training.
- **Offline replay (`c4243b5`).** `--replay-corpus` (repeatable) + `--epochs` run a dedicated headless train-only mode (no self-play, arena, or promotion). `CorpusReplayFeeder` replays games through the exact `ActiveGame` staging + reverse-ply flush path self-play uses, so a replayed position is byte-identical to the original; `CorpusReplayRunner` step-locks the SGD loop, feeding `K = batchSize / replayRatioTarget` positions per step. Params are snapshotted on the main actor into a Sendable struct so the off-actor GPU work never touches the `@MainActor` singleton. (`--start-model` for replay is not yet wired — a fresh net is used; the cross-arch A/B use case works today.)
- **Provenance (`461c95c`).** Additive Optional `recordingCorpusID` on `SessionCheckpointState` + a `[RESUME-PARAM] recording_corpus_id` line; `results.json` carries `recording_corpus_id`. A resumed run starts a fresh corpus rather than appending (the non-latest fork/warn flow stays a ROADMAP follow-up).
- **PGN import (`049e94f`).** `--import-pgn` streams a `.pgn`/`.pgn.zst` (via `zstd -dc`), matches each SAN token against the engine's legal moves, and replays through `ChessGameEngine` into a corpus. Standard-start games only; filters `--min-rating` / `--max-games` / `--min-plies` / `--time-control`.
- **Usage (`5a93c54`).** The `--help` banner gains an offline-replay / PGN section with worked examples for the four common scenarios (cross-arch A/B, same-init hyperparameter A/B, Lichess import, multi-epoch replay).

Same day: two self-play chart tiles — game length + draw rate from the rolling `ParallelWorkerStatsBox` snapshot (`fd8cee5`) — and a batch of code-review fixes (`3016060`): the `value_label_smoothing_epsilon` pre-feature resume resolver, a status-checked split working-weight-sync GPU pass, `ModelCheckpointFile.encode` refusing to write a `.dcmmodel` the decoder can't resolve, a stable `InputEncoding` header tag on the replay buffer (reusing the former zero pad — no version bump), one-time preset-folder scan caching in Build-New-Model, and the wide-set OVERALL summary fold cached per snapshot.

## 2026-06-19 CDT — Network: feature-skip (long concat skip) architecture axis (`449f625`)

Optional, defaulted-off `FeatureSkip` on `NetworkArchitecture`: a single long concat skip carrying the stem output to the policy and/or value heads and/or the final block. Two fusion modes — `concat_direct` (each routed consumer reads `concat([tower_out, source])`, widening its own first conv in place; no new tensors) and `compress_conv_bn_relu` (one shared 1×1-conv→BN→act compresses the concat back to tower width, head-only). Off configs build **byte-identical**: same `weightTensorPlan`, `parameterCount`, and golden summaries, and all existing saved models load unchanged. Adds the `v4_5block_7x7_fusion` preset, Build-screen controls + diagram marker, and dual-contract tests (`parameterCount == plan element sum`) for both modes and the final-block destination. Also forces `ARCHS=arm64` for all configs (the x86_64 archive build hit an Intel-only `Float16.bitPattern` in `writeScalarFeed`; this app is Apple-Silicon-only) and caps the About popover in one `ScrollView` so the architecture diagram no longer clips.

## 2026-06-18 CDT — Testing: full suite in the default test plan (`dbf6ee5`)

Adds `DrewsChessMachine.xctestplan` with no selected/skipped tests so the test target runs **everything**, wired into the scheme + project, and drops the stale single-test scheme selection. Records in `macos27-beta1-mpsgraph-findings.md` that the full suite is green on macOS 26.5.1 (808 passed, 0 failed, 1 env-gated skip) while the `MacOS27NaNIsolation` / `FP16ComputePath` tripwires fail by design on macOS 27 beta 1.

## 2026-06-17 CDT — fp16 entropy-path NaN fixes + 7-day code-review pass (`98f1a4d`, `0d3cfa8`, `169d2ce`, `3d94a05`, `aac15f7`)

Follow-ups to the fp16 entry below. Two **universal fp16 underflow NaNs** in the policy-entropy path fixed (`98f1a4d`, not macOS-27-specific): the illegal-move masked-logit bias `-1e9` overflows fp16 (max finite 65504) to `-inf`, giving `0·(-inf)=NaN` at every legal cell — use a dtype-representable `-3e4` under fp16, `-1e9` byte-identical for bf16/fp32; and the entropy `eps=1e-7` is an fp16 denormal that flushes to zero (`log(0)=-inf`) — compute the entropy log-path in fp32. With both fixed the fp16 forward, entropy, and total loss are finite; a separate fp16 **backward** gradient overflow remains (deferred for re-baselining on a pre-macOS-27 device). The **7-day code-review pass** (`0d3cfa8`) hardens the fp16/block-groups/dropout/picker window: the legal-mass alarm no longer clobbers the divergence-suspend banner; `dropoutRate` surfaced in `results.json`; `pushDropoutRateToGraph`'s `graph.run` serialized under `weightAccessLock`; the SE `maxTensorElementCount` ceiling raised to `2·c·c`; the block-group editor keyed on stable UUIDs; an atomic single-winner claim shared across the termination paths; plus new `DropoutGraphWiringTests` (scaffolding present in training, absent in inference) and `FP16Bf16NarrowEquivalenceTests`. Concurrency/cleanup: `pushDropoutRateToGraph` reads its MPS scaffolding through `self.network` to clear nine Swift 6 capture diagnostics (`169d2ce`), `SessionPickerModel`'s scan closure captures `self` weakly (`3d94a05`), and a dead `dtype` binding in `feedsForBatch` is removed (`aac15f7`).

## 2026-06-17 CDT — fp16 (float16) compute precision: inference-ready, training NaNs (`b25f37e`)

`ComputeDataType` gains a `.float16` case alongside `.float32` / `.bFloat16`, selectable per model in Build-New-Model (it's a `ComputeDataType.allCases` picker, so it surfaced with no UI change) and embedded in the safetensors metadata. The plumbing was already ~80 % present — `.float16` branches existed in `makeWeightData` / `decodeWeightData` / the array-form `readFloats` / `bytesPerWeightElement` / `weightRelativeEpsilon` — so this wires the four remaining touchpoints: the `mpsDataType(for:)` mapping (ternary → exhaustive switch), the `readFloats(into:)` hot-path readback (vImage `Planar16FtoPlanarF`, previously a `fatalError`), and the trainer's `writeRealValuedFeed` / `writeScalarFeed` host-side narrows (vImage bulk / native `Float16` scalar). The in-graph casts and the fp32-master mixed-precision optimizer path are dtype-generic (keyed on `dtype != .float32`), so fp16 inherits them unchanged; config-D `bf16CastInForward` correctly stays bf16-only. A full audit confirmed no hardcoded `.bFloat16` cast targets leak into the fp16 path.

**(FINDING) fp16 is inference-only on this stack.** A real fp16 forward pass is finite and well-formed, and an fp16 model round-trips bit-exact through the safetensors save (incl. the forward-verify gate) / load path — both tested and passing. But a real fp16 `trainStep` diverges to a **NaN gradient on the very first step at every batch** (worse than bf16, whose batch-1 cell is finite): the forward CE components come back finite while the aggregate loss and gradient norm are NaN, so the overflow is in the fp16 backward and/or an auxiliary loss term (entropy / illegal-mass), not the fp32-accumulated CE reductions. fp16's exponent range is far narrower than bf16/fp32; viable fp16 training would need loss scaling and/or fp32 computation of the loss + aux terms. This and every other MPSGraph issue on the Xcode 27 b1 / macOS 27 beta stack are now collected in `documentation/macos27-beta1-mpsgraph-findings.md`.

**Tests.** `FP16ConversionTests` (vImage-vs-native narrow agreement across ties/overflow/subnormals, `makeWeightData`↔`decodeWeightData` roundtrip, both `readFloats` overloads, `mpsDataType` exhaustiveness, metadata, JSON persistence) all pass. `FP16ComputePathTests` forward-smoke + safetensors-roundtrip pass; the two `trainStep` cells **fail by design** as documented tripwire markers (pinned out of the default scheme run, like the `MacOS27NaNIsolationTests` cells) — they flip green if fp16 training is ever made viable.

## 2026-06-13 CDT — Code-review fixes: empty-block_groups crash, NaN-multiplier trap, dropout-resume regime, picker edge cases (`54e3ca3`)

High-effort review of the 4-day block-groups/dropout/picker window turned up several bugs, now fixed with regression tests (729 tests green). Correctness: (1) `NetworkArchitecture.init(from:)` now **rejects an empty `block_groups` array with a thrown `DecodingError`** instead of decoding an arch whose stem/head accessors `preconditionFailure` — those accessors are read by `SessionManifest.extract` (on the picker's background queue) and the `SafetensorsModelIO` load path **before** `validate()`, so a malformed `session.json`/`.safetensors` previously crashed the app (picker scan, `--probe-model`, `--start-model`) instead of surfacing a catchable error. (2) `validate()` rejects a non-finite dropout multiplier via a new `mustBeFiniteNonNegative(Float)` error case; the old code built the error with `Int(multiplier)`, which **traps on exactly the NaN/∞ values it meant to reject**. (3) `dropout_rate` resume gained a pre-feature fallback (`resolvedDropoutRate`, = 0 when `saved == nil`) matching momentum/illegal-mass/etc., so resuming a pre-dropout session no longer injects the live rate into a run that never used it. Session-picker display fixes (all in `SessionManifest`): partial draw-counter block reports nil (not a misleadingly-low total); empty `arenaHistory` no longer masks the probe's arena count; `pElo` walks back to the last **finite** tick; promotion-count fallback tolerates a non-`[[String:Any]]` arena-history shape. `maxTensorElementCount` now bounds the `fc_bottleneck` policy FC so a valid such model isn't rejected on load.

The review also confirmed (refuted as non-bugs): the `post-promotion` log label vs the single-token `promote` on-disk trigger (folder-name parse is correct); the deliberate GPU `status == .error` check (`834d72c`); and that legacy `.dcmmodel` archHash collisions for mixed towers are unreachable (mixed towers persist only as safetensors with the full embedded config).

## 2026-06-12 CDT — Block groups: heterogeneous towers with per-group widths + architecture diagram (`73a1bdd`, `120f46b`)

The tower is now an ordered list of **block groups** — each a count plus a complete block recipe (channels, both conv kernels, SE style/ratio, ReZero + α-init, activation function + style, skip merge, per-group dropout multiplier) — replacing the ten uniform block fields on `NetworkArchitecture`. The engine consumes only the flattened `expandedBlocks`; groups are authoring/persistence structure. Width transitions (WRN staircase) insert a bias-free **1×1 skip projection** consuming the shared pre-activation (He v2) on pre-act blocks, appended LAST in the block's tensor order — mirrored exactly by `weightTensorPlan`, so uniform towers keep today's layout bit-for-bit (the five presets' historical parameter counts and tensor counts reproduce with those tests unmodified). Encode writes only `block_groups`; legacy uniform keys decode forever into a single group, so every existing file loads unchanged. `architectureSummary` now renders EVERY attribute of every group explicitly (no silent defaults; golden-string tests pin the form). Dropout gains one mask-shape tensor per distinct width and a per-group multiplier composed with the live rate variable (multiplier 1 adds zero ops). Analyzer fan-ins/shapes/drift references derive from each block's own spec. Phase B: the Build-New-Model screen gets a per-group editor (move/duplicate/remove/add, per-group α mismatch flag vs 1/√(total blocks)) beside a live **ArchitectureDiagramView** — width-proportional cells, full per-group recipes, orange projection markers, per-segment parameter counts summed from the weight plan — shared by the draft pane and the About popover's live-network view. New GPU tests build + evaluate a mixed 64→128 tower and construct the trainer's full gradient graph through the projection. Design history (incl. stride added-and-dropped same day, two-level groups simplified to one level) recorded in `ARCHITECTURE_EXPANSION_PLAN.md`.

## 2026-06-12 CDT — Rich Load Session picker: manifest-at-save, legacy index cache, lineage grouping (`9939563`)

File > Load Session now opens a custom picker (SESSION_PICKER_PLAN.md) instead of the bare `.fileImporter` — the importer stays reachable via **Browse…** for sessions outside the default directory. Saves are grouped by run lineage (the ModelID tag parsed from the folder name), newest run first; each row shows date, trigger badge (fixed semantic colors: manual=blue, periodic=gray, promote=green, sigusr2=orange), steps, arenas/promotions, wide pElo, build, and disk size; the detail pane carries the four manifest categories (architecture, run/progress/lineage, performance, hyperparameters-at-save) plus Reveal in Finder. Load funnels into the same `session.loadSessionFrom(url:)` path as the importer. Data source: `CheckpointManager` writes a small `manifest.json` into every new `.dcmsession`, derived from the **exact** `session.json` bytes just written (single extraction code path; non-fatal on failure since the manifest is regenerable). Legacy sessions get indexed once from their ~16 MB `session.json` on a background queue, cached **outside** the immutable session folders (`…/DrewsChessMachine/SessionIndex/`, validated by size+mtime), and delivered to the sheet incrementally. Unreadable sessions render as visible error rows, never silently dropped. New tests: `SessionManifestTests` (UTC folder-name parsing, legacy + runtime-config architecture extraction, promotion-count fallback, manifest round-trip, unreadable-row identity) and `SessionPickerModelTests` (group/within-run ordering, unreadable retention, scan-directory URL derivation).

## 2026-06-12 CDT — (FINDING) Dropout A/B v2 from the trained 5K7Z champion: still no distinguishable effect at 600 steps

The four-arm repeat (0.00A / 0.00B replicate / 0.30 / 0.70, 600 steps each from the step-~498k 5K7Z champion via `--start-model`) landed the same null result as the random-init v1, now with a calibrated noise floor: full-trajectory wide-probe NLL deltas vs the control mean were −0.0005 (rate 0.30) and −0.0053 (rate 0.70), while the 0.00A/0.00B control pair itself split ±0.0028 with per-tick spread ~0.013–0.02; mean pElo 886–892 across all four arms. Masks were demonstrably active (mechanical training-side signatures differ by arm) and rate 0.70 remained completely stable on a trained net. Conclusion: the dropout machinery is validated end to end, but any regularization *benefit* is a long-horizon (tens-of-thousands-of-steps) question that a 600-step probe cannot answer.

## 2026-06-12 CDT — (DESIGN) Block-groups plan revised: two-level groups, per-block strides, explicit summaries (`f57fd62`)

`ARCHITECTURE_EXPANSION_PLAN.md` Feature 2 restructured: `BlockSpec` (full per-block recipe including both conv kernels **and strides**) → `BlockGroupEntry` (spec × count) → `BlockGroup` (name + entries + repeatCount) → tower, with the engine consuming only the flattened `expandedBlocks` list — groups are authoring/persistence structure only, and the skip-projection rule becomes purely per-block (any width change or stride product > 1). The first-block-only stride restriction is removed in favor of per-block (H, W) bookkeeping (ceil-division, floored at 1×1) + FC-policy-head validation. Summaries and the architecture diagram render every attribute of every entry explicitly (no silent defaults), with golden-string tests pinning the new form and a one-time old→new mapping note for ARCH_EXPERIMENTS.md. Uniform-tower bit-exact identity (existing checkpoints pass their embedded forward-pass verification under the new builders) is now a named validation item.

## 2026-06-12 CDT — Channel dropout (live-tunable) + headless A/B harness (`eacced3`, `600c016`)

Every training-mode residual block now carries a **channel (spatial) dropout** node at the WRN slot (between the conv2-side activation and conv2): one `[N, C, 1, 1]` Philox draw per block per step, keep-mask `u >= rate`, inverted `1/(1−rate)` scaling. Inference graphs contain no dropout scaffolding at all, so champion/arena/probe paths are untouched and the bit-exact save verification doubles as proof. The rate lives in a graph **variable** (no new feed requirements anywhere) and is a new live-tunable `DropoutRate` training parameter (drop probability, PyTorch/Keras convention — the description documents the retention-vs-drop trap): popover row under Decay, out-of-band graph assign onto the live trainer, `drop=` in the `[STATS]` reg group, optional `.dcmsession` field with three-way `[RESUME-PARAM]` hydration. Rate 0 is an exact identity at unmeasurable cost (step timing indistinguishable from the pre-dropout build). One field crash en route: deriving the mask shape via `shapeOf(stem output)` attached a gradient-less consumer to the trainable path and MPSGraph's autodiff hard-asserted at trainer build — the shape now derives from the input placeholder, and `DropoutGraphSemanticsTests` (rate-0 identity, slab masking + exact scaling, expectation preservation, gradient-equals-scaled-mask) locks the construction. Granularity decision record with rejected alternatives (unit/pinhole, DropBlock, stochastic depth) and references through 2023 lives in `ARCHITECTURE_EXPANSION_PLAN.md`.

The headless A/B harness: `--start-model <path>` loads a saved model as the starting champion under `--train` (trainer forks from it — N runs from one identical starting net), and `--training-step-limit <steps>` stops a run at an exact step budget (snapshot + exit, `termination_reason: step_limit_reached`). Both validated the same day across seven headless runs.

## 2026-06-12 CDT — (FINDING) Dropout A/B: masks verified active; ≈ zero effect at 600 steps from random init

Three 600-step arms (rates 0 / 0.30 / 0.70) from one random-init champion via `--start-model`, fully autonomous (SIGUSR2 checkpoint of the live session, arms, pointer restore, auto-resume). Plumbing verified end-to-end (per-arm `[PARAM]` graph application, `drop=` telemetry, byte-identical starting weights confirmed by matching first probe ticks); **rate 0.70 trains completely stably** (gNorm ~1.47, no entropy collapse) with ~38 of 128 channels surviving per draw. Across full trajectories with a noise-floor comparison, **no arm is distinguishable from control**: an infant net has no channel structure for dropout to destroy. A four-arm repeat from the *trained* 5K7Z champion (with a 0.00 replicate to calibrate run-to-run noise) was launched the same day. Reminder this finding bakes in: with continuous fresh self-play data there is little overfitting for classic always-on dropout to fight (see Liu et al. 2023, "Dropout Reduces Underfitting", for the modern scheduling reframe — reproducible by hand with the live-tunable rate).

## 2026-06-12 CDT — Wide pElo status cell, visible chip spinner, chart step labels (`6e4e233`, `ec43a27`)
...
**1. Residual tower doubled 8 → 16 blocks.** `ChessNetwork.numBlocks` 8 → 16. Parameters ~2,483,667 → ~4,934,867 (the tower is ~98% of the net, so the count nearly doubles; the policy-head change below accounts for the rest). At the default batch 4096, training-activation RAM roughly doubles (~5 GB → ~10 GB, unified memory) and per-step train time / per-move self-play latency each ~2×. Found and fixed a latent bug while doing this: the tower build loop and two loops in `NetworkWeightAnalyzer` hardcoded `0..<8` even though `numBlocks` existed — all now keyed off `ChessNetwork.numBlocks`, so the analyzer reports all 16 blocks instead of silently truncating to 8.

**2. Zero-γ identity init on each block's `bn2`.** Standard deep-ResNet "zero-γ" init: the last BN in every residual block initializes gamma to 0 (via a new `zeroInitGamma` flag on `ChessNetwork.batchNorm`) instead of 1, so each block starts as identity (`ReLU(input + 0)`) and the tower output equals the stem output regardless of depth. The conv weights underneath stay random He-init — they contribute nothing to the forward pass until gamma trains off zero.

*Why it was needed.* This is a **single-stage** tower (constant 128ch / 8×8, no downsampling stages), so unlike a multi-stage ImageNet ResNet there are no stage boundaries to re-normalize the residual skip-sum — variance accumulates monotonically across blocks. At 8 blocks the one-shot BN warmup masked it; at 16 it did not. Measured on a fresh 16-block `.randomWeights` net: inference-mode `legalMass/uniform` meanRatio collapsed to **0.112** (healthy 0.5–1.5), and the warmup-calibrated per-layer `running_var` climbed monotonically 0.8 → 3.9 across the tower. Training-mode meanRatio stayed healthy (0.73), confirming it was an inference/warmup-calibration surface of a depth-driven root cause, not a trainer defect — but a 0.112 init policy is the no-signal-self-play failure mode that can stall bootstrapping. Zero-γ kills the accumulation at the source (depth-independent init) and gives cleaner gradient flow from step 0. (One consequence: the residual-branch weights' gradient scales with γ, so while γ ramps up from zero their first updates fall below fp32 ULP and don't visibly move — additive params like betas/biases get order-1 gradients and do move, but the multiplicative weights don't budge yet. That's a property of the init, not a wiring bug. The `testEveryTrainableVariableUpdatesAfterOneStep` connectivity gate — renamed `testEveryTrainableVariableReceivesGradient` — now primes every `bn2` γ to 1.0 before its one-step probe, so full-magnitude gradient flows through every branch; a genuinely disconnected variable still receives exactly zero gradient and is caught.)

**3. Policy head gains a `conv → BN → ReLU` stage.** The policy head was a bare 1×1 conv on the raw tower output (no normalization), while the value head already had `conv → BN → ReLU`. That asymmetry is exactly why the value path stayed finite but the policy path went degenerate under accumulated tower variance. Inserted an intermediate 1×1 conv (128→128, no bias) → BN → ReLU before the final 1×1 conv (128→76), mirroring the value head and lc0's convolutional policy head; the final conv still emits raw logits (no BN/activation after — logits need free scale for the softmax). With change #2 this is belt-and-suspenders, but it brings the two heads into structural parity. Policy-head params 9,804 → 26,700.

Supporting/bookkeeping: added `ChessNetwork.parameterCount` (derived from the arch constants) and pointed the About popover's architecture + parameter lines at it instead of stale hardcoded text; `NetworkWeightAnalyzer.deterministicInit` now references 0.0 for `*_bn2_gamma` (drift-from-init correctness) and `fanIn` learned `policy_pre_conv_weights`. Save/restore, weight copies (promotion / trainer fork / arena & probe snapshots), and trainer optimizer (velocity) state were verified to scale automatically off the dynamic `trainableVariables` / `bnRunningStatsVariables` lists with count validation at load — no hardcoded tensor counts.

Also resolved test breakage on the way (all pre-existing on `main`, unrelated to the architecture except where noted): `TrainingParametersTests.test_registry_size` 43 → 46 (params added since the counter was last bumped); two `FENParserTests` malformed-input cases that were committed with wrong expectations (`wrongFieldCount` 5 → 4; a rank-overflow FEN that actually trips `rankFileSumMismatch` rather than `unknownPieceCharacter`); `RepPlaneProbeTests` now `XCTSkip` when no architecture-compatible champion exists on disk (they load a real champion, which the `archHash` change correctly rejects); and `testKAwareStratumSizingHandlesScarceDecisiveBuffer` was rewritten for the true-ceiling sampler (the old scenario asserted pre-true-ceiling fill-to-target behavior) to exercise the genuine degradation path — a decisive-scarce buffer whose decisive deficit reflows into draws.
...
**Why.** Threefold repetition is currently the dominant draw cause (~36% of all terminations in the build-1082 run). The existing planes 18–19 carry only the *count* of how many times this position has occurred (saturated at 2); they cannot express *when* those recurrences happened. The new planes let the network distinguish a 2-ply shuffle (bits 1 and 3 set after two visits) from a longer maneuvering cycle, which is the structural shape of the draw mode dominating the buffer. Cost is negligible (~+0.5% network compute, +2.6 GB replay-buffer storage on 1M positions) so this is the cheapest high-leverage experiment available; if it doesn't move threefold-draw share, the data itself points the next attack at conversion/endgame technique rather than repetition signal.

**Files / what.**
- **`Encoding/BoardEncoder.swift`** — `GameState` gains a `recentRepetitionMask: UInt16` field (default 0) carrying the packed 10-bit signal; `withRecentRepetitionMask(_:)` is the engine-side setter that mirrors the existing `withRepetitionCount(_:)`. `BoardEncoder.encode` reads `state.recentRepetitionMask` and fills planes 20–29 via the existing skip-if-zero broadcast pattern. Existing call sites (tests, UI editable position, `MPSChessPlayer`'s per-ply encode, the Forward Pass demo, `CheckpointManager`'s verification probe) need no source changes — synthetic `GameState`s construct with the default mask=0 and produce zero new planes, which is the correct "no recent repetitions" encoding for non-game-tracked states.
- **`Chess/ChessGameEngine.swift`** — new `recentPositionKeys: [PositionKey]` window, max length 10, most-recent first. Maintained inside `applyMoveAndAdvance` next to the existing `positionCounts` table: the pre-apply state's `PositionKey` is prepended after each move and the window is truncated to 10; on irreversible moves (halfmove clock = 0) the window is cleared along with `positionCounts`. The mask is computed by `PositionKey`-comparing each window entry against the new state's key and is layered onto the state via `withRecentRepetitionMask`. Exposes `static let recentPositionKeyWindow = 10` so tests can read the same constant the implementation uses.
- **`Network/ChessNetwork.swift`** — `inputPlanes = 20` → `30`. Stem weight shape becomes `[128, 30, 3, 3]` automatically; `arch_hash` (computed from input/output shape signature) shifts so old `.dcmsession` / `.dcmmodel` files fail to load with a shape mismatch — no migration code (per project policy on breaking architecture changes).
- **`Training/ReplayBuffer.swift`** — per-position storage grows from 5.1 KB to 7.7 KB (no code change, the `floatsPerBoard` constant references `ChessNetwork.inputPlanes`); 1M-position buffer goes from 4.9 GB → 7.5 GB. Comment updated to reflect the v3 architecture.
- **`documentation/chess-engine-design.md`** — plane table extended from 18 to 30. The doc was already stale at 18 (out of date since the 20-plane `repetitionCount` planes shipped); this brings the headline count and the table into sync with reality, including planes 18–19 it had been missing.
- **`CLAUDE.md`** — input description in the "Board encoding and policy space" section updated from 20-plane to 30-plane.
- **Tests** — new `DrewsChessMachineTests/RepetitionPlaneTests.swift` (10 tests): all planes zero at game start; bit-3 / plane-23 set after a single Nf3 Nc6 Ng1 Nb8 knight cycle (current position == position 4 plies ago); bits 3 and 7 set after the cycle is run twice; zero-padding when history is shorter than 10 plies; mask cleared on a pawn move (halfmove clock reset); window rebuilds correctly after the clear; `PositionKey` distinguishes opposite-STM-but-identical-pieces; `withRecentRepetitionMask` round-trips and other fields preserve; encoded planes mirror the set bits exactly; arch sanity (`tensorLength == inputPlanes × 64`, `recentPositionKeyWindow == 10`). Two existing tests had hard-coded `1280` literals that snapshot the v2 plane count (`BoardEncoderTests.testTensorLength` and `PolicyHeadCorrectnessTests.testEncoderTensorLengthMatchesNetworkInputShape`); both updated in lockstep to `1920` with explicit user approval, comments adjusted to reflect v3.

**Not changed.** No backwards compatibility shim — existing `.dcmsession` and `.dcmmodel` files cannot load into the v3 build (loud shape-mismatch error at load time, as the loader's validator already does). No width/depth/head-structure changes. No sampling-parameter or loss-formulation changes. The architecture change requires a fresh training run.

---

## 2026-05-12 — Composition-aware replay-buffer batch sampler (3 per-batch constraints + composition readout)

`ReplayBuffer.sample(count:…)` previously did `count` independent uniform draws over the resident ring — blind to which game a position came from, the game's length, or its outcome. On a draw-saturated buffer (~85% of resident positions from drawn games; ~370-ply shuffle marathons) that means the trainer's minibatch is mostly `z = 0` positions from long aimless games, so the W/D/L value head's target carries almost no position information, the policy-gradient baseline is ≈0 everywhere, and `advantage = z − vBaseline ≈ z` degenerates. This adds per-training-batch composition control plus an O(1) pre-constraint composition readout — the most direct lever on that degeneracy that doesn't require the champion to improve first (it changes what the *trainer* sees; it does **not** change the buffer's underlying draw rate, which is champion-strength × sampling-temperature and is not affected by `draw_penalty` either).

**Files / what.**
- **`Training/TrainingParameters.swift`** — three new `@TrainingParameter` keys in the `Replay Buffer` category, all `liveTunable`: `max_plies_from_any_one_game` (Int, default `10`, range `1...400` — the cap is **on by default**; with typical batch sizes (e.g. 4096) it is always active in this range), `target_sampled_game_length_plies` (Int, default `0` ⇒ off), `max_draw_percent_per_batch` (Int, default `100` ⇒ off). Registry 36 → 39 (`TrainingParametersTests.test_registry_size` and the file's header comment updated; `CLAUDE.md` "36 tunable parameters" → 39). Persisted values that fall outside the new range (the previous default `65536` from the parameter's earlier `max_samples_per_game_per_batch` name) fail validation on read and fall back to the new default — so an existing user's per-game cap silently turns ON to `10` at next launch. The earlier short-lived `max_samples_per_game_per_batch` key shipped before the rename; treat it as never released.
- **`Training/ReplayBuffer.swift`** —
  - *Composition aggregates* maintained incrementally under the buffer lock (in `append`'s insertion/eviction passes, alongside `hashStats`, and rebuilt in `restore`): per-class resident-position counts (`winPositions`/`drawPositions`/`lossPositions`), `sumGameLengthOverResidentPositions`, a `[UInt32:Int]` resident-game refcount (`distinctResidentGames = .count`), and a `[UInt16:Int]` resident length histogram. Exposed via `compositionSnapshot() -> CompositionSnapshot` (game-weighted and position-weighted mean game length, W/D/L fractions; the position-weighted mean is `E[L²]/E[L]`, the gap between the two is the length dispersion). Observability-only — not persisted; rebuilt on `restore` (no on-disk format bump, stays v7). Known wart documented: across a *resumed* session the per-worker game index resets, so an old resident game can collide with a new one and be merged for counting/capping — harmless.
  - *`SamplingConstraints`* struct (`maxPerGame` / `maxDrawPercent` / `targetMeanGameLengthPlies`, `.unconstrained` ⇒ the legacy sampler, `@MainActor fromCurrentParameters()`), stored on the buffer (`samplingConstraints`, set via `setSamplingConstraints(_:)`). Owned here rather than passed per call so the off-main trainer never reads `TrainingParameters.shared`; the `ChessTrainer.sample(...)` call sites are unchanged.
  - *`sample(...)` rewrite*: a `isNoOp(forBatchSize:)` fast path that is **bit-for-bit the legacy uniform-with-replacement sampler** at default settings; otherwise stratified-rejection on outcome for an exact `maxDrawPercent` ceiling (the draw stratum's freed slots go to decisive positions; if either stratum is short of supply the split clamps and the cap simply under-shoots), an exponential `exp(−β·max(0, L−T))` down-weight for the soft `targetMeanGameLengthPlies` (β solved per call by bisection on the resident length histogram — `solveLengthTiltBeta`; β = 0 when the untilted mean is already ≤ T), and a per-batch `[UInt32:Int]` tally for the exact `maxPerGame` cap. Hard/soft priority: the draw cap and per-game cap are hard, the length target yields first; a bounded attempt budget falls back to a uniform fill rather than hang or return a short batch.
- **`App/SessionController.swift`** — new `bufferComposition: ReplayBuffer.CompositionSnapshot?` mirrored property.
- **`App/SessionController+Heartbeat.swift`** — each tick: `replayBuffer?.setSamplingConstraints(.fromCurrentParameters())` (idempotent, one lock op) + dirty-checked mirror of `compositionSnapshot()` into `bufferComposition`.
- **`App/SessionController+Training.swift`** — seed the constraints once at Play-and-Train start (no ≤5 s unconstrained window); add `comp=(games=… lenG=… lenP=… W=… D=… L=…)` to the `[STATS]` line (pre-constraint composition, distinct from the post-constraint `[BATCH-STATS]` line).
- **`App/UpperContentView/UpperContentView.swift`** — a `Composition: len/game=… len/pos=… W/D/L=…%/…%/…%` line under the existing `Buffer:` line in the training stats column.
- **`App/UpperContentView/TrainingSettingsPopover.swift` + `…Model.swift`** — a new "Replay sampling" section at the bottom of the Replay tab: three control rows (text + stepper, the standard `PopoverRow` pattern) — "Max plies from any 1 game" (hint `plies from 1 game`), "Target avg game plies" (hint `plies`), "Max draws % per batch" (hint `% per batch`, switches to `off — no draw cap` at 100). Wired into the model's `…Text`/`…Error` fields, the Replay-tab error aggregation, `reload`/`resetErrors`/`save` (with `[PARAM] …` log lines). Below the controls, a one-fact-per-line composition readout — games stored, per-game and per-sampled-position mean game length, W/D/L %, and the within-decisive +z/−z balance (orange when outside `[45, 55]` — a sign-assignment / colour-imbalance smell).
- **`parameters.json`** regenerated.
- **Tests** — new `DrewsChessMachineTests/ReplayBufferSamplingConstraintsTests.swift`: no-op constraints produce a valid uniform batch and `compositionSnapshot` matches a known append sequence; `maxDrawPercent` is an exact ceiling and freed slots are decisive; `maxPerGame` is never exceeded in a batch; `targetMeanGameLengthPlies` pulls the sampled mean down toward the target, is monotone in the target, is a no-op at 0 / above the natural mean, and never undershoots the shortest resident game; composition aggregates survive a `write`→`restore` round-trip; under-fill returns `false` and an empty stratum clamps gracefully.

**Why.** The buffer's draw glut has no clean lever (a stronger champion needs promotions; the temperature schedule is the only direct knob; a ply cap just turns long games into truncated draws) — but the *trainer* doesn't have to be fed the buffer's raw composition. Capping draws and de-weighting shuffle marathons per batch gives the value head a target with real position content and de-degenerates the advantage signal without waiting on a champion improvement. The two newer levers (draw cap, length target) ship neutral so the user opts in; the per-game cap ships **on at 10** because at a 4096-batch any cap ≥ 4096 is structurally a no-op, and ≤ 400 ensures the batch always spans ≥ 10 distinct games — the actual reason the cap exists.

**Not changed.** Self-play / the replay buffer's contents (champion-driven). The replay-ratio controller (it counts positions consumed/produced — composition doesn't change the count). The probe sampler in `ChessTrainer` (kept at the no-op path — it's a diagnostic forward, not a training step). No achieved-vs-requested reporting beyond the existing `[BATCH-STATS]` line for v1.
...
**Files / what.**
- **`Training/ReplayBuffer.swift`** (commit `b48793c`) — dropped the dead per-slot `vBaselines` (Float32) column (on-disk format `v6 → v7`): the W/D/L rewrite made the play-time-frozen baseline obsolete (the trainer recomputes the policy-gradient baseline from a fresh forward pass every step), so nothing was left to persist. `append`/`sample` lose the `vBaselines:` param; `MPSChessPlayer` stops passing it; the trainer keeps its `replayBatchVBaselines` staging buffer (now filled entirely from the fresh forward). Older replay-buffer files are rejected on load.
- **`value_label_smoothing_epsilon` into the session lifecycle + UI** (commit `5ecfeab`) — `SessionCheckpointState` gains an Optional `valueLabelSmoothingEpsilon` (old session files decode to nil → fall through to the user's current `TrainingParameters` value, like its policy-side sibling); `buildCurrentSessionState` writes it and the resume path restores it onto both the trainer and `TrainingParameters.shared` with a `[RESUME-PARAM]` log line; the Optimizer tab of `TrainingSettingsPopover` gains a "Value label smoothing:" row (range `[0, 0.5]`, step `0.05`) with the model's text/error/seed/reset/validate-write-back plumbing.
...
## 2026-05-12 — Value head: scalar `tanh` → 3-way W/D/L softmax + categorical cross-entropy (`4c00983` … `29b8597`)

**Files.** `Network/ChessNetwork.swift` (`valueHead` now `FC(64→3)` raw W/D/L logits in `[win, draw, loss]` slot order, `value_fc2_bias` init `[0, ln 6, 0]`, **no tanh**; returns `(scalar, logits, probs)` where `scalar = Σ_c softmax(logits)_c·[+1,0,−1]_c = p_win − p_loss`, shape `[batch,1]` — same as the old tanh scalar, so every inference readback is byte-identical; new `let valueLogits` / `let valueProbs` exposed for the trainer; new `static let valueHeadClasses = 3`). `Training/ChessTrainer.swift` (`buildTrainingOps`: value loss MSE-on-tanh-scalar → categorical CE over `network.valueLogits` vs an in-graph one-hot `oneHot(clamp(int(1 − z), 0, 2), depth: 3)` — `idx = 1 − z` maps z=+1→0, z=0→1, z=−1→2 — optionally label-smoothed by a new `value_label_smoothing_epsilon` scalar placeholder (`target = (1−ε)·oneHot + ε·(1/3)`), via `softMaxCrossEntropy(reuctionType: .none)` then mean; the advantage/REINFORCE path is **unchanged** — `advantage = z − vBaseline`, the `vBaseline` placeholder still fed the fresh trainer-forward's scalar each step; 3 new diagnostic tensors `valueProbWin/Draw/Loss` (batch means of the softmax columns, via `sliceTensor`); `valueMean`/`valueAbsMean` kept, now mean / mean-abs of `p_win − p_loss`; the `value_label_smoothing_epsilon` placeholder is wired through the same plumbing pattern as `policy_label_smoothing_epsilon` — return tuple, both inits, NDArray + TensorData, `buildFeeds` write, `feedsForBatch` dict; `TrainStepTiming`: drop `vBaselineDelta`, add `valueProbWin/Draw/Loss`; `TrainingLiveStatsBox`: drop `_vBaselineDeltaWindow`/`rollingVBaselineDelta`, add `_valueProbWin/Draw/LossWindow`/`rollingValueProbWin/Draw/Loss`; readback gains 3 slots; `trainStepFromReplay` drops the stale-vBaseline copy + `meanAbsDelta` diagnostic). `App/SessionController+Training.swift` (`[STATS]` line: `vBaseDelta=` → `pW=/pD=/pL=`; CliTrainingRecorder feed updated). `CLI/CliTrainingRecorder.swift` (`v_baseline_delta` → `value_prob_win/draw/loss`). `App/UpperContentView/TrainingAlarmController.swift` (value-head "saturation" alarm de-tanhed: thresholds 0.97/0.999 kept but reframed as "calling nearly every position a clean win/loss"; the real WDL collapse — `pD → 1` — shows up directly on `[STATS]` as `pD=`). `Training/TrainingParameters.swift` (new `@TrainingParameter ValueLabelSmoothingEpsilon` — id `value_label_smoothing_epsilon`, default `0.0`, range `[0, 0.5]`, category `Optimizer`, `liveTunable`; registry 35→36). `App/SessionController.swift` (start path passes / live-sets `valueLabelSmoothingEpsilon`). `Persistence/ModelCheckpointFile.swift` (`currentArchHash` mixes `ChessNetwork.valueHeadClasses` so `.dcmmodel` / `.dcmsession` files saved against the scalar head are cleanly rejected — the value-head variable shapes changed). `parameters.json`. Tests: new `DrewsChessMachineTests/ValueHeadWDLTests.swift` (7 standalone-MPSGraph tests pinning the value-CE math, the slot mapping incl. fractional draw penalty, label smoothing, the derived scalar = `p_win − p_loss` and its `[−1,1]` range, the `[0, ln6, 0]` bias-init draw-heavy-prior / zero-scalar invariant, and that MPSGraph's float→int cast truncates); `CliTrainingRecorderTests` fixture + minor stale-comment cleanup in `TrainingAlarmControllerTests` / `TrainingParametersTests`. Docs: `documentation/chess-engine-design.md` (value-head section update callout), `wdl-value-head.md` (design), `wdl-implementation-log.md` (work log). 356/356 tests green.

**Why.** On the build-893 fresh-from-random run (the 2026-05-11 15:30 FINDING above), the scalar `tanh` value head trained vs `z ∈ {−1, 0, +1}` with MSE *collapsed to ≈0* on the ~64–85%-draw self-play buffer — `vAbs` fell from ~0.30 (init) to ~0.017 and bottomed, `vMean ≈ 0`, `vLoss` flat — i.e. it learned "everything is a draw" and stopped producing useful gradient. The arena candidate plateaued at ≈parity-minus-a-bit for ~7+ hours (76 arenas, 0 promotions): all the (glacial) policy improvement was pure outcome-weighted imitation with the AlphaZero value amplifier missing. A 3-way win/draw/loss classifier keeps a usable gradient because the draw class is just one of three competing logits — the categorical-CE target stays informative on draws (`p_draw → 1`) without crowding out confident win/loss on the ~36% decisive games. The scalar head can't do both. See `wdl-value-head.md` for the full math and rationale; `sampling-parameters.md` §"draw_penalty" remains the anti-draw / contempt lever (deliberately not a new knob).

**Scope notes / deferred follow-ups.** The inference path stays scalar (`p_win − p_loss`) — move selection is policy-only and the only inference-time value consumers (dashboard, candidate probe, the trainer's fresh-baseline forward) want the scalar; the 3-wide logits/probs live only in the training graph + metrics. The `ReplayBuffer`'s play-time `vBaseline` storage and the `vBaselines:` params on `append`/`sample` are **kept but now dead** (their only consumer was the removed `vBaselineDelta` diagnostic; the trainer's `vBaseline` placeholder is fed entirely from the fresh trainer-forward) — a clean follow-up removal that would ripple into `MPSChessPlayer` self-play. `value_label_smoothing_epsilon` is **not yet persisted in the `.dcmsession` saved-config block** (default 0 = no-op; it still persists via `TrainingParameters`' UserDefaults). The value-loss is now CE-scale (~[0, ln 3 ≈ 1.1] at convergence, transiently larger), not the old MSE scale (~0.1–0.4) — a `vLoss` curve compared across the switch is not apples-to-apples, and `value_loss_weight = 1.0` may want lowering for the first WDL run (live-tunable, no rebuild). Old `.dcmmodel` / `.dcmsession` files no longer load (arch-hash mismatch — expected).

**What to watch in the first WDL run.** A fresh net starts at `pW ≈ pL ≈ 0.125`, `pD ≈ 0.75`, `vMean ≈ 0`, `vAbs ≈ 0` (the `[0, ln6, 0]` bias). Healthy training pulls `pD` *down* from 0.75 and lets `pW`/`pL` differentiate per position (`vAbs` rising off the floor) — that's the value head finally doing something. The collapse signature to watch is `pD → 1.0` (equivalently `vAbs → 0` and staying there): if it happens, the WDL head has rediscovered "everything is a draw" in the new representation; a small positive `draw_penalty` and/or `value_label_smoothing_epsilon` are the levers. `vLoss` should settle below ~`ln 3` once the head is calibrated. The advantage / policy-side diagnostics (`pIllM`, `pEnt`, `gNorm`, `playedMoveProbPosAdv − …NegAdv`, etc.) are unchanged by this and should behave as in the 2026-05-11 FINDING.

---

## 2026-05-11 — Replace arena-only "Promote Trainee" with a standalone "Promote Trainee Now" (no arena)

**Files:** `App/DrewsChessMachineApp.swift` (Engine menu: drop `Promote Trainee`, add `Promote Trainee Now`, enabled when `realTraining && !isArenaRunning`). `App/AppCommandHub.swift` (`promoteCandidate` → `promoteTrainerNow`). `App/UpperContentView/UpperContentView.swift` (drop the `arenaOverrideBox?.promote()` wiring; add `promoteTrainerNowFromMenu()` with belt-and-suspenders guards + a `.confirmationDialog` that says the weights are not arena-validated). `Arena/ArenaOverrideBox.swift` (now abort-only — `OSAllocatedUnfairLock<Bool>`, `abort()` / `isActive` / `consume() -> Bool`; the `Decision` enum and `promote()` are gone). `App/SessionController+Arena.swift` (the post-tournament branch is now `let aborted = overrideBox.consume()` → `shouldPromote = !aborted && playedGames >= totalGames && score >= threshold`; arena promotions are always `promotionKind == .automatic`). `Arena/PromotionKind.swift` (doc updated — `.manual` now means "user invoked Promote Trainee Now", `gamesPlayed == 0`). `Persistence/SessionSaveTrigger.swift` (new `.manualPromote` case — disk tag `promote`, UI suffix ` (post-promotion)`). `App/SessionController+Checkpoint.swift` (`saveSessionInternal` is now `internal` so the manual-promote path can reuse it). `App/SessionController+ManualPromote.swift` (new — `promoteTrainerNow()`). `ROADMAP.md`.

**Why.** `Engine ▸ Promote Trainee` was an *in-flight-arena override only* — disabled unless an arena was running, it forced promotion of the arena candidate ignoring the score, and it didn't act until the in-flight arena games drained (the tournament driver only checks the override flag between games, and never cancels running games), so it routinely looked like "nothing happened". The semantics were confusing and rarely what's wanted: if an arena is running you can just let it finish (it promotes on its own if the candidate wins) or Abort it; if you want the *current trainer* in as champion right now, that's the new action.

**New behavior.** `Engine ▸ Promote Trainee Now` (enabled while Play-and-Train is running and no arena is in flight) raises a confirmation dialog noting the weights are un-arena-validated; on confirm, `SessionController.promoteTrainerNow()` pauses self-play + training (bounded waits), copies the live trainer weights into the champion on a detached GPU task (errors surfaced, never swallowed), re-stamps IDs (champion inherits the trainer's ID; trainer rolls forward to a fresh forked generation — same rule as arena promotion), resumes both gates, appends a synthetic `TournamentRecord` (`gamesPlayed == 0`, `promotionKind == .manual`) + a chart marker, resets game-play stats, logs `[STATS] post-promote(manual-no-arena) …`, and — gated on `autosaveSessionsOnPromote` — writes a `-promote.dcmsession` via the shared `saveSessionInternal`. Deliberately does **not** touch trainer optimizer velocity, `completedTrainSteps`, `trainingBox`, or alarms: unlike the arena path (which rewinds the champion to an arena-start snapshot and so must restore the matching velocity/step count), this promotion takes the trainer's *current* state verbatim, so the trainer's accumulators are already correct for the weight surface both networks now share — training continues seamlessly.

**Tests.** No existing test referenced the removed `ArenaOverrideBox.Decision.promote` / `commandHub.promoteCandidate`; `PromotionKind` is still exercised by `ArenaLogFormatterTests` / `TournamentRecordTests` and stays valid.

---

## 2026-05-11 15:30 CDT — FINDING: build 893 (`5b2005a`) fresh-from-random run — legal mask solved, no divergence, candidate at ≈parity, value head silent — line in the sand before refactor (FINDING)

**Run:** `~/Library/Logs/DrewsChessMachine/dcm_log_20260511-020022.txt`, build 893, git `5b2005a` (the load-bearing fix exercised by this run is `56bc90a` "Policy CE back over raw logits"; `5b2005a` is its immediate successor and the head the run launched on). Launched 2026-05-11 02:00:22 CDT — **fresh build** (`[BUTTON] Build Network` → `[BUTTON] Play and Train`, no resume), lineage `trainer=20260511-5-ASg9-1 / champion=20260511-5-ASg9`. Run ongoing as of this entry; numbers below are the state at ≈13h09m / step ≈23,400 / 49 auto-arenas / 0 promotions. **No source change in this entry — this is the milestone record before the in-progress refactor.**

**Parameter state for the run** (`[STATS] reg=` block + config line): `batch=4096`, `lr=1.0e-4` (flat — `sqrt_batch_scaling_lr=false`), `momentum μ=0.90`, `weight_decay=1e-4`, `grad_clip_max_norm=30`, `policy_loss_weight=1.0`, `value_loss_weight=1.0`, `entropy_bonus=1.5e-2` (a brief mid-run excursion to 5e-3 → 1.0e-2 around hour 11, reverted to 1.5e-2 — a non-event), `illegal_mass_weight=1.0`, `draw_penalty=0.0`, `policy_label_smoothing_epsilon=0.1`, `lr_warmup_steps=500`, `arena_auto_interval_sec=900`, `arena_games=400`, `promote≥0.55`, `replay_buffer_min_positions_before_training=345678`, 16 self-play workers.

**Where it got:**
- `pIllM 0.9922 → ~0.16`; `legalMass 0.005 → ~0.85` (training batch), legal-mass probe on fresh positions ~0.86–0.88. **The `56bc90a` "what to watch" prediction held** — `pIllM` fell from ~0.99 toward ≪0.1 — except it *plateaued* around 0.16 (the `illegal_mass_weight=1.0` / `entropy_bonus=1.5e-2` equilibrium), short of the ≪0.1 target. Cosmetically short of zero; functionally fine (`top1Legal` ~0.90–0.95).
- **No divergence.** `gNorm` bounded ~1.3 the entire run (clip=30 never triggered — ~23× headroom); `pLogitAbsMax` ~13.9 (plateaued); `pwNorm` Δ ≈ +0.05 from init (~12.44, essentially flat). The `19d8ab6` / `4ef1a26` / `419c07f` / label-smoothing / `56bc90a` guardrail stack holds. **First run that is neither diverging (build 875) nor stalled (builds 879/888).**
- **Arena: candidate reached ≈parity with the frozen-random champion.** Score crept ~0.40 (arenas #2–#15, hours 2–5) → ~0.44 (#16–#41, hours 7–12) → ~0.47 (#42–#49, hour 13), with arena #45 touching `0.5062` (elo +4, CI [−15,+24]) — the high-water mark. Above parity as White throughout the late run (`cand_white` ~0.50–0.53), ~0.45–0.48 as Black — a persistent ~0.05–0.08 White edge, larger than chess's intrinsic White advantage. No promotion in 49 arenas (threshold 0.55). Improvement rate at hour 13 ≈ +0.03 arena-points/hr.
- **Value head silent — the central limitation.** `vAbs` collapsed from ~0.30 (init) to ~0.021 and bottomed; `vMean ≈ 0`; `vLoss ≈ 0.14` flat. On a ~64%-draw random-self-play buffer the scalar `tanh` value head learns "everything ≈ a draw" and provides essentially no gradient — so **all the (glacial) policy improvement above is pure outcome-weighted imitation; the value amplifier the AlphaZero loop relies on is missing.** Motivates a WDL (win/draw/loss softmax) value head + a small contempt / `draw_penalty` for the next iteration: a WDL head's categorical-CE target stays informative on draws (`p_draw≈1`) without crowding out confident win/loss on the ~36% decisive games; the scalar head can't.
- `pLossLoss = +0.0000` the entire run — expected: the `max(0, advantageNormalized)` gate (2026-05-10 17:00 entry) zeros the policy-CE contribution of negative-advantage positions, so the loss-conditional CE term is structurally zero. Not a bug; documented there. The right-direction signal that *did* appear: `playedMoveProbPosAdv − playedMoveProbNegAdv` widened from ~0 to ~0.027 over the run (the policy slowly learning to prefer moves from good-outcome positions).
- **Slow-erosion side effects (neither fatal, both ongoing):** (1) the outer replay-ratio compensator's target drifted `1.0 → ~2.3` over 13h (same shape as build 879's drift to 4.34, ~10× slower), driven by the 15-min/400-game arena cadence (each arena pauses both self-play and training; the compensator over-corrects on the dip) — self-play throughput fell ~39% from the ~9.4 M pos/hr post-warmup peak. (2) `[TICK-SLOW]` grew from ~50 ms at session start to ~5.5 s steady (build 879's heartbeat-stall curve at ~half-rate), with ~9 s spikes during the 4-hourly checkpoint writes (1M-position `.dcmsession` save takes >10 s); the `__processSnapshotTimerTick` structural fix (drop the per-stage `withCheckedContinuation` / `DispatchQueue.global` dispatches) is still pending. (3) the `.dcmsession` filename modelID (`20260511-6-EUai`) does not match the active champion modelID (`20260511-5-ASg9`) — cosmetic naming wrinkle, functionally harmless (the loader reads networks from inside the file, not the name).

**Conclusion / next-iteration targets.** Build 893 + these params is the high-water mark: stable, legal-mask solved, candidate at ≈parity, but no promotion yet and the value head is dead — the from-scratch bootstrap is just barely working, policy-side only, at ~+0.03 arena/hr. Targets for the next iteration: **(a) WDL value head + small contempt** — the real fix for the silent-value-head bottleneck; **(b) lower and/or annealed entropy bonus** — the fixed `1.5e-2` actively fights policy concentration (`pEnt` climbed `1.06 → 1.85` over the run; the ~50-min `1.0e-2` excursion visibly slowed that), **and a higher base LR** — `gNorm` at ~1.3 vs `clip=30` is ~23× headroom for faster learning; **(c)** the **`__processSnapshotTimerTick` heartbeat fix** and a **wider `arena_auto_interval_sec`** (relieves the compensator drift and gives the candidate more training between arenas); **(d)** optionally a higher `illegal_mass_weight` to finish the legal-mask phase in ~2h instead of ~4h (cosmetic — 1.0 was fine).

---

## 2026-05-11 — Policy CE back over raw logits (revert `acc5340`'s CE masking): fixes illegal-mass saturation at ~0.997

**Files:** `Training/ChessTrainer.swift` (`buildTrainingOps`: the fused `softMaxCrossEntropy` now receives `network.policyOutput` instead of `maskedLogits`; the `--- Policy loss ---` comment block gains a third structural-piece paragraph documenting the masked-CE backfire; the "No per-position CE clamp" block and the masked-entropy NOTE updated to spell out that the CE needs raw logits while the entropy bonus needs masked ones, and that the raw-logit CE's illegal-cell gradient is `softmax_raw(illegal) ∈ [0,1]` — bounded, so no new divergence vector; `maskedLogits` is still built and still feeds the entropy bonus, move selection, and the legal-cell diagnostics — only the CE input changed). `DrewsChessMachineTests/PolicyLossIllegalMassGradientTests.swift` (new — builds a faithful fragment of the policy loss and asserts the sign/magnitude of ∂loss/∂(illegal logit): raw-logit CE puts a strong positive gradient there (→ driven down) and a strong negative gradient on the played cell (→ driven up); masked-logit CE leaves ~no gradient on illegal cells, ~two orders of magnitude weaker — the documented pre-fix counter-case). `CHECK_NEXT.md` (resolution section).

**Symptom.** `[STATS]` across the last several sessions/resumes: `pIllM=0.9967` (99.67% of the unmasked policy softmax on illegal moves), `legalMass≈0.003`, `pLossWin≈+9.1` (worse than uniform over the 4864-cell head), all flat. The probe showed one move-type-shaped illegal cell (e.g. "f4-e6") as the top policy output with 70%+ across unrelated positions. A freshly built net legitimately starts ~99.4% illegal (softmax uniform over 4864 cells, ~30 legal) and is supposed to learn that down toward ≈0; it didn't — it parked at the init value.

**Root cause.** `totalLoss = w_v·valueLoss + w_p·policyLoss − entropyCoeff·policyEntropy + illegalMassWeight·illegalMassPenalty`. Three of the four terms gave ~zero gradient on illegal logits: `policyLoss` (CE) was over `maskedLogits = policyOutput − 1e9·illegalMask` (commit `acc5340`, "Adds legal move masking during training runs … prevents mass from accumulating on illegal moves, I hope" — it did the opposite), so `∂CE/∂(illegal logit) ≈ softmax_masked − target ≈ 0 − 0 = 0`; the entropy bonus is over the masked softmax (correct — the *unmasked* entropy bonus rewards spreading mass onto illegal cells, which is what `19d8ab6` fixed and must stay); the softmax-mass `illegalMassPenalty` (weight 1.0) has `∂/∂logit ∝ p`, which → 0 once illegal mass ≈ 1 — it saturates at exactly the pathology it exists to fix; decoupled weight decay pulls all logits toward 0, not toward legal cells. So nothing pushed illegal logits down. The fully-conv policy head (1×1 conv 128→76, per-channel bias shared across all 64 squares) compounds the visible "one cell everywhere" because a single inflated channel bias raises that move type on every square — but the head shape isn't the cause; the missing training signal is.

This is **distinct** from the divergence analysed in `CHECK_NEXT.md` (logit blowup → `pLoss = −64,868`), which the `19d8ab6` / `4ef1a26` / `419c07f` fixes cured — `pLogitAbsMax=6.9`, `gNorm=1.1`, `pLoss=+0.43` are all healthy in the affected runs. This is the *opposite* failure: bounded everything, but stuck at near-uniform illegal mass.

**Fix.** Feed `network.policyOutput` to the CE (revert `acc5340`'s change to the CE input only). The smoothed target `(1−ε)·oneHot(played) + ε·uniform(legal)` is zero on every illegal cell, so the CE can never *reward* illegal mass — masked or not — but now `∂CE/∂(illegal logit) = softmax_raw(illegal) − 0 = softmax_raw(illegal) ∈ [0, 1]`: a bounded gradient that always pushes illegal logits down and vanishes only once illegal mass is genuinely ≈ 0 (the correct fixed point). The CE *value* can be transiently large (up to ≈ log(policySize) ≈ 8.5 while raw mass is mostly on illegal cells), but a large CE value is not a large update — the per-logit gradient `softmax_raw − target` is hard-bounded in [−1, 1] regardless of concentration — so no graph-level clamp is needed and stability is unaffected (if anything improved, since the illegal-logit gradient actively drains the mass that would inflate `pLogitAbsMax`).

**`illegal_mass_weight` left at 1.0** — it's now largely redundant (the unmasked CE's legal-only target already drives illegal mass to zero), but it's harmless (stable attractor at 100% legal mass, gradient always points down on illegal logits, and it gets *relatively stronger* as illegal mass falls: `∂/∂logit ∝ softmax·(1 − illegalMass)`). It's a live `TrainingParameter`, so lowering or zeroing it later is a config change with no rebuild. Not retuned here so the CE-unmask experiment isn't confounded.

**What to watch in the next training run.** With a freshly built net: `pIllM` should fall from ~0.994 toward ≪0.1 over the first few thousand steps (today: flat at 0.9967); `legalMass` should rise from ~0.003 toward ~1. `pLossWin` starts ~9 and should *decrease* (not stick). `playedMoveProb` should rise from ~0.25 toward the ε=0.1 equilibrium ~0.90. `pLogitAbsMax` and `gNorm` should stay bounded (sanity check vs. the `CHECK_NEXT.md` divergence — this change shouldn't reintroduce it). The "f4-e6 everywhere" probe behaviour should disappear once illegal mass drains. Optionally: resuming the current collapsed `.dcmsession` post-fix should claw `pIllM` back from 0.997 (the gradient on the inflated illegal logits is now ~`softmax_raw(illegal)`, currently large) — a fresh run is the clean validation either way.
