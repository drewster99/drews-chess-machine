...
- **Guards against destructive re-runs** — `migrate v5` rebuilt the CSV from the markdown doc and would have **deleted all 761 imported rows**, and separately blanked the 80 measured `elapsed_train_sec` values for segments 0–2 (whose logs died with the deleted VM, so SegTime resolves them to ""). It now rebuilds only doc-sourced segments, carries later segments through untouched, and never replaces a real reading with a blank. `recompute` would likewise have moved `elapsed_train_sec` while leaving `wall_sec` stale, making their difference a meaningless "clamp measurement"; both now move together. Both verified in a `DCM_DASH_ROOT` sandbox.
