...
- **Enumerated checkpoints are named from the out-model stem, and every resumed segment gets its own stem** (`…-Qeu8-replay-step*`, `…-Qeu8-resume2-replay-step*`, …). `registry.json` names only the *latest* via `out_model`, so a single glob is blind to every earlier segment. Each segment may therefore declare `enum_stem`; `enum_specs()` resolves one glob per segment and falls back to the old single glob when none is declared. `python3 replay.py discover-stems [run] [--write]` derives them from the checkpoints themselves, **by evidence rather than by filename**: a stem's suffix must be resume-shaped (which keeps a sibling run like `Qeu8e5` out of `Qeu8`), every file under one stem must agree on `model_id`, and the segment is then chosen by how many of the stem's files land on `cum_step`s the CSV already knows, tie-broken by which segment's length best matches the stem's extent, with all stems required to map onto segments monotonically in creation-time order. It refuses rather than guesses when that is ambiguous. **Do not infer the segment from the suffix** — qeu8's segment 0 predates enumeration, so its first enumerated segment is 1 and it wrote under the *unsuffixed* stem; trusting the name there invents 68 rows at cum steps that never existed. **Run it on the machine that holds `Models/`** — it reads `models_dir`, which is deliberately `~/…` so it resolves locally wherever the tracker runs.
