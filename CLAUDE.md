...
- Network: stem (30→128) → 12 pre-activation residual blocks (each with an SE channel-attention module, reduction ratio 4) → fully-convolutional policy head (1×1 conv 128→76) + value head (1×1 conv 128→16 → BN/ReLU → flatten(1024) → FC 1024→128 → ReLU → FC 128→3 W/D/L logits) → ~3.9M parameters. See `ChessNetwork.swift`. SE blocks match modern lc0 practice; the fully-conv policy head replaces the prior FC bottleneck head.
