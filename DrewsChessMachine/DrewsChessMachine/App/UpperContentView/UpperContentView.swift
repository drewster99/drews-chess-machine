...
struct UpperContentView: View {
    ...
    var body: some View {
        ...
        .onReceive(snapshotTimer) { _ in
            ...
            Task.detached {
                await session.processSnapshotTimerTick(dispatchedAt: dispatchedAt)
            }
        }
    ...
    private func handleBodyOnAppear() {
        ...
        session.onSyncMenuCommandHubState = { syncMenuCommandHubState() }
        session.onUpdateGameSnapshot = { gameSnapshot = await gameWatcher.asyncSnapshot() }
        session.isBuildingOrBusyProvider = { isBuildingOrBusy() }
        session.onClearInferenceResult = { inferenceResult = nil }
    ...
    // Checkpoint save/load (handleSaveChampionAsModel / handleSaveSessionManual /
    // handleSaveSessionPeriodic / saveSessionInternal / loadModelFrom / loadSessionFrom +
    // their pick-result handlers + the saveGateTimeoutMs static) moved to SessionController
    // in Stage 5a — callers go through session.*.
    ...
    private func wireMenuCommandHub() {
        ...
        commandHub.saveChampion = {
            ...
            session.handleSaveChampionAsModel()
        }
    ...
    // championLoadedSinceLastTrainingSegment moved to SessionController in Stage 5a — proxy.
    private var championLoadedSinceLastTrainingSegment: Bool {
        get { session.championLoadedSinceLastTrainingSegment } nonmutating set { session.championLoadedSinceLastTrainingSegment = newValue }
    }
