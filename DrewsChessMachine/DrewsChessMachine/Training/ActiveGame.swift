...
final class ActiveGame: @unchecked Sendable {
    ...
    /// Flush **only one colour's** recorded plies to the replay buffer,
    /// with every row's outcome signed from that colour's perspective.
    ///
    /// This is the train-vs-UCI path: that driver records only the
    /// trainer's plies (on-policy, outcome-only), so exactly one side's
    /// scratch is populated and the other side's `*PliesRecorded` is 0.
    /// The two-sided `flush()` above cannot be used there — it sources
    /// each game-ply by parity and would read the *unrecorded* opposite
    /// side's scratch for half the rows. This method sources rows only
    /// from `trainerColor`'s contiguous per-side scratch (indices
    /// `0..<recorded`), which `recordPly` already stamped with the
    /// correct game-total `plyIndex` (white on even plies, black on odd),
    /// so the emitted positions are valid regardless of the opponent's
    /// unrecorded moves. Rows are written in reverse recorded order to
    /// match `flush()`'s convention; ordering within a game block is
    /// cosmetic for the replay buffer.
    /// - Parameter totalPlies: the true total half-move count of the
    ///   game (both sides). Stamped as `gameLength` so replay metadata,
    ///   analyzers, and target-length sampling see the real game length
    ///   rather than the trainer-only recorded-row count (which is ~half).
    func flushTrainerSide(
        buffer: ReplayBuffer,
        result: GameResult,
        trainerColor: PieceColor,
        totalPlies: Int
    ) -> FlushedGameStats? {
        let recorded = (trainerColor == .white) ? whitePliesRecorded : blackPliesRecorded
        ...
        let outcome: Float = (trainerColor == .white) ? whiteOutcome : -whiteOutcome

        let bf = boardFloats
        let srcBoard = (trainerColor == .white) ? whiteBoardScratch : blackBoardScratch
        let srcPolicy = (trainerColor == .white) ? whitePolicyIndices : blackPolicyIndices
        let srcPly = (trainerColor == .white) ? whitePlyIndices : blackPlyIndices
        let srcTau = (trainerColor == .white) ? whiteSamplingTaus : blackSamplingTaus
        let srcHash = (trainerColor == .white) ? whiteStateHashes : blackStateHashes
        let srcMat = (trainerColor == .white) ? whiteMaterialCounts : blackMaterialCounts

        var phaseByPly = PhaseHistogram.zero
        var phaseByMaterial = PhaseHistogram.zero
        for dst in 0..<recorded {
            let src = recorded - 1 - dst
            let dstBoard = mergedBoardScratch + dst * bf
            dstBoard.update(from: srcBoard + src * bf, count: bf)
            mergedPolicyIndices[dst] = srcPolicy[src]
            mergedPlyIndices[dst] = srcPly[src]
            mergedSamplingTaus[dst] = srcTau[src]
            mergedStateHashes[dst] = srcHash[src]
            mergedMaterialCounts[dst] = srcMat[src]
            mergedOutcomes[dst] = outcome
        ...
        buffer.append(
            ...
            stateHashes: mergedStateHashes,
            materialCounts: mergedMaterialCounts,
            gameLength: UInt16(min(totalPlies, Int(UInt16.max))),
            workerId: workerId,
            intraWorkerGameIndex: intraWorkerGameIndex,
            outcomes: mergedOutcomes,
            count: recorded
