...
#### Implementation notes (bootstrap phase)

Two details of `ChessTrainer.buildTrainingOps` that the math above doesn't capture:

**Policy loss is scaled ×1000 relative to the value loss** in `totalLoss = valueLoss + 1000 · policyLoss`. The pre-MCTS bootstrap is structurally a REINFORCE update on a 4096-way softmax using the played-move one-hot as its policy target, and the per-logit gradient magnitude works out to roughly `1/(N · batch)` — three orders of magnitude weaker than the value head's `(z − v)²` gradient. Without the weight, the value head converges normally (`vLoss` ≈ 0.9 → 0.07 over the first few hours) while the policy head barely moves at all: policy entropy stays pinned at `log(4096) ≈ 8.297` nats to four decimal places across thousands of training steps, and `pLoss` oscillates around zero because the signed `z`-weighted contributions from winning and losing positions almost cancel in the mean. Scaling by 1000 puts the two heads into the same order of magnitude during the bootstrap. This weight is a bootstrap-phase hack and should be revisited when MCTS visit-count targets replace the one-hot — the visit-count target is itself a much stronger signal (per-position loss of O(log N_legal_moves) concentrated on the actually-explored moves, rather than a ±scalar smeared across the full 4096-wide space) and will not need the same aggressive rescaling.

**Use `MPSGraph.softMaxCrossEntropy` for the policy loss, not a manual log-softmax.** A hand-rolled `softMax → log → multiply → sum` chain has a practical failure mode in fp32: once the strong `×1000` gradient starts pushing logits around, individual softmax outputs underflow to exactly `0.0`, `log(0) = −∞` propagates through the multiply, and both `pLoss` and `pEnt` print as `NaN`. The obvious fix — stable log-softmax via `x − max(x) − log Σ exp(x − max(x))` — compiles fine but **crashes inside `gradientForPrimaryTensor:` at graph-construction time** with `"Op gradient not implemented"`, because MPSGraph's autodiff has no gradient implementation for `reductionMaximum`. The fused `softMaxCrossEntropy(_:labels:axis:reuctionType:name:)` op (note Apple's typo in the Swift name) sidesteps both problems: it is numerically stable by construction and ships its own `softMaxCrossEntropyGradient` implementation, so autodiff never tries to walk the internal softmax. Feed it the raw logits + one-hot labels with `reuctionType: .none`, reshape the `[batch]` result to `[batch, 1]`, then apply the `z` weighting and mean reduction exactly as in the math above.
