...
*Implementation deviations from this design (all logged in `wdl-implementation-log.md`): (1) the slot index gets a `clamp(·, 0, 2)` before `oneHot` (defensive, unreachable with the current `draw_penalty` range but cheap insurance — mirrors the policy path's `max(|legal|, 1)`); (2) `pW/pD/pL` (the W/D/L softmax batch-means) were added as the value-head diagnostics, and `vMean`/`vAbs` were kept (now mean / mean-abs of `p_win − p_loss`) rather than fully replaced; the W/D/L distribution is exposed at inference only via a dedicated `ChessNetwork.evaluateValueDistribution(board:)` (for the candidate-probe / Run Forward Pass panel), not on the universal inference closures (those still carry just the scalar — a deliberate scope cut, expand if a feature ever needs the distribution on the hot path). Two deviations from the "implemented" status above were since closed: `ReplayBuffer`'s dead `vBaseline` column / `vBaselines:` params were removed (on-disk format `v7`, commit `b48793c`), and `value_label_smoothing_epsilon` is now persisted in the `.dcmsession` saved-config + restored on resume + has an Optimizer-tab UI control (commit `5ecfeab`). A `pD → 1` value-head-collapse alarm was also added (the failure mode this whole change targets). See the `2026-05-12 — WDL follow-ups` CHANGELOG entry.*
