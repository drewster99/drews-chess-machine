...
final class ChessTrainer: @unchecked Sendable {
    ...
    private func buildFeeds(
        ...
        // Early-training entropy boost: linearly decay an extra
        // multiplier on the entropy regularization coefficient from 3×
        // at step 0 down to 1× at warmup end. The graph subtracts
        // `entropyCoeff * policyEntropy` from total loss, so a larger
        // coefficient pushes the policy harder toward uniform when the
        // network is most fragile and prone to collapse onto a single
        // illegal move (build-393 high-variance regime). Once warmup
        // completes (`warmupMul == 1`), the boost factor becomes 1 so
        // steady-state behavior matches the user-tuned parameter.
        // Composes with the LR warmup ramp: while LR is still climbing,
        // entropy pressure is concurrently elevated, so the early
        // small-but-noisy updates have a built-in regularizer against
        // logit blowups. Skip the boost entirely when warmup is
        // disabled to keep that opt-out path identical to before.
        let entropyBoostFactor: Float
        if lrWarmupSteps > 0 {
            entropyBoostFactor = 1.0 + 2.0 * (1.0 - warmupMul)
        } else {
            entropyBoostFactor = 1.0
        }
        var entropyCoeff = entropyRegularizationCoeff * entropyBoostFactor
