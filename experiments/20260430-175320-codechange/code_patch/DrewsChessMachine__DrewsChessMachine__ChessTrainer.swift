...
final class ChessTrainer: @unchecked Sendable {
    ...
    private static func buildTrainingOps(
        ...
        // --- Advantage std floor for long-run stability ---
        //
        // The previous formulation used `var + 1e-6` purely as a
        // divide-by-zero guard. That keeps the math finite but does
        // nothing about a more insidious late-training failure mode:
        // when the value head saturates (vAbs → 1.0) or a batch happens
        // to land on positions with one dominant outcome, the batch
        // advantage variance collapses by 100×+. Dividing by that tiny
        // std then magnifies the per-row REINFORCE weight by the same
        // factor, producing rare-but-huge policy-gradient spikes that
        // the global L2 clip flattens but cannot fully neutralize —
        // SGD's running step direction absorbs the outlier and the
        // policy starts collapsing toward whichever logit was
        // over-pushed by the spike. This is precisely the late-training
        // illegal-mass-collapse signature this engine fights against.
        //
        // Floor the variance at `varFloor = 0.04` (std floor of 0.2)
        // via `max(var, varFloor)` rather than `var + eps`. Under any
        // healthy bootstrap batch, raw advantage std sits comfortably
        // in the 0.4 – 1.0 range (z ∈ {−1, 0, +1} mixed with
        // `vBaseline ∈ (−1, 1)`), so this floor is a no-op for almost
        // every step and only engages when std has already collapsed —
        // exactly the late-training regime the long-run goal asks us
        // to protect against. Choosing `max` over `addition` also
        // keeps the floor exact rather than additive (which slightly
        // biases std upward in the healthy regime).
        //
        // Orthogonal to all tunable parameters: this is a numerical-
        // stability guard on the loss formulation itself, not a knob
        // achievable via entropy_bonus / weight_decay / lr_warmup.
        let advantageVarFloor = graph.constant(0.04, dataType: dtype)
        let advantageVarPlusEps = graph.maximum(
            advantageVarForNorm,
            advantageVarFloor,
            name: "advantage_var_floored"
        )
